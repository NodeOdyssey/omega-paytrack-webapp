...
import EditRequiredRanks from './EditRequiredRanks';
...
import EditScheduleEmployee from './EditScheduleEmployee';
...
const postValidationSchema = Yup.object().shape({
  ...
  // phoneNum: Yup.string(),
  // .nullable() // Allows the field to be null or empty without triggering validation
  // .test('phoneNum', 'Phone Number must be 10 digits', (value) => {
  //   if (!value || value.trim() === '') return true; // Skip validation if empty
  //   const phoneRegex = /^\d{10}$/;
  //   return phoneRegex.test(value) && !value.includes('-'); // Validate only if input is present
  // }),
...
const EditPostForm: React.FC = () => {
  ...
  /* Fetch Edit Post Id from URL */
  const urlParams = new URLSearchParams(window.location.search);
  const editPostIdParams = urlParams.get('id');

  // Fetch Post Data on Page Load
  useEffect(() => {
    setEditPostId(Number(editPostIdParams));
    fetchPostData(Number(editPostIdParams));
  }, [accessToken]);
  ...
  const fetchPostData = async (postId: number) => {
    // Check if postId and accessToken are available
    ...
    try {
      ...
      // If response is successful, set postFormData, postDocsFormData
      if (response.data.success) {
        setEditPostName(response.data.post.postName);
        setPostFormData({
          ...response.data.post,
  ...
  const handleSubmitPostForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        handleSubmitRequiredRanks(requiredRanks, editPostId);
      }
      if (postFilesChanged) {
  ...
  const [existingPostRankLinksData, setExistingPostRankLinksData] = useState<
    PostRankLink[]
  >([]);
  // const [clearSelectedRanksData, setClearSelectedRanksData] = useState(false);
  ...
  const fetchPostRanks = async () => {
    ...
    if (!editPostId || !accessToken) return;
    setIsLoading(true);
    try {
      const response = await axios.get(
        `${api.baseUrl}${api.posts}/${editPostId}/link-rank`,
        {
          headers: {
            'x-access-token': accessToken,
          },
      ...
      try {
        for (const postRank of selectedRanksData) {
          ...
          if (response.data.success) {
            ...
            fetchPostRanks();
            // TODO: Schedule Employees should be updated here
            setRefreshPostRanksNeeded(true);
          }
        }
      } catch (error) {
        handleAxiosError(error as AxiosError);
  ...
  const handleSubmitPostDocs = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!accessToken || !editPostId) return;
    setIsLoading(true);
    try {
      if (!editPostId || !postDocsFormData || !postFilesChanged) return;
      postDocsFormData.postName = editPostName;
      const response = await axios.post(
        `${api.baseUrl}/posts/${editPostId}/upload`,
        postDocsFormData,
        {
          headers: {
  ...
  const [refreshPostRanksNeeded, setRefreshPostRanksNeeded] = useState(false);
  const resetRefreshPostRanksNeeded = () => {
    setRefreshPostRanksNeeded(false);
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-2.5 2xl:gap-4 fixed z-20 padding-responsive-header-container-type-1">
        ...
        <div className="flex">
          ...
          >
            ...
            />
            {/* <h2 className="primaryHeadings text-secondaryText text-sm lg:text-base">
                Back
              </h2> */}
            {/* <ClearButton>Back</ClearButton> */}
      ...
      <div className="bg-white mt-[70px] 2xl:mt-[92px] z-10 overflow-y-auto h-[calc(100vh-164px)] 2xl:h-[calc(100vh-208px)] scrollbar scrollbar-thumb-scrollBarColor scrollbar-track-scrollBarBg">
        <form
        ...
        >
              ...
              <div className="flex justify-between gap-4 2xl:gap-6">
                ...
                <div className="flex flex-col gap-1 2xl:gap-2 w-1/4">
                  <label className="input-label flex flex-row justify-between">
                    <div>
                      Contract Date <span className="input-error pl-1">*</span>
                    </div>
                    <div>
          ...
          {/* Accordion 2 */}
          <Accordion title="Required Ranks">
            <EditRequiredRanks
          ...
          <EditScheduleEmployee
            ...
            postId={editPostId}
            refreshData={refreshPostRanksNeeded}
            resetRefreshData={resetRefreshPostRanksNeeded}
          />
          {/* Schedule Buttons */}
          ...
          <Accordion title="Upload Documents">
            <div className="flex w-full items-center gap-4 2xl:gap-6 pb-4 2xl:pb-6 border-b">
              <div className="flex justify-between gap-4 2xl:gap-6 w-full">
                {/* Contract */}
                <div className="flex flex-col gap-1 2xl:gap-2 cursor-pointer w-1/4">
                    ...
                    typeof postDocsFormData.docGst === 'string' ? (
                      <>
                        ...
                        >
                          ...
                          />
                          <span className="text-labelColour font-medium text-responsive-input overflow-hidden text-ellipsis">
                            {extractFileNameFromPath(postDocsFormData.docGst) ||
                              'Upload Document'}
                          </span>
                        </a>
                        <button
                    ...
                    typeof postDocsFormData.docPan === 'string' ? (
                      <>
                        ...
                        >
                          ...
                          />
                          <span className="text-labelColour font-medium text-responsive-input overflow-hidden text-ellipsis">
                            {extractFileNameFromPath(postDocsFormData.docPan) ||
                              'Upload Document'}
                          </span>
                        </a>
                        <button
          ...
          <div className="flex justify-center gap-6 py-4 mb-4">
            <SecondaryButton onClick={useBackButtonHandler()}>
              Back
            </SecondaryButton>
