...
/* Edit Rank Form Main Component */
const EditRankForm: React.FC = () => {
  /* Scroll To Top */
  useScrollToTop();
  ...
  const editRankId = urlParams.get('id');

  /* Edit Rank Form Data Handling */
  const [rankFormData, setRankFormData] = useState<Rank>(defaultRankFormData);
  const fetchCurrentRankData = async () => {
    try {
      const response = await axios.get(`${api.baseUrl}/ranks/${editRankId}`, {
        headers: {
          'x-access-token': accessToken,
        },
      });
      if (response.data && response.data.success) {
        const rank: Rank = response.data.rank;
        if (rank) {
          const existingRankData = {
            designation: rank.designation,
            basicSalary: Number(rank.basicSalary),
            hra: Number(rank.hra),
            kitWashingAllowance: Number(rank.kitWashingAllowance),
            cityAllowance: Number(rank.cityAllowance),
            conveyance: Number(rank.conveyance),
            vda: Number(rank.vda),
            uniformAllowance: Number(rank.uniformAllowance),
            otherAllowance: Number(rank.otherAllowance),
            specialAllowance: Number(rank.specialAllowance),
            weeklyOff: Number(rank.weeklyOff),
          };
          setRankFormData(existingRankData);
        }
      } else {
  ...
  useEffect(() => {
    if (!editRankId || !accessToken) return;
    fetchCurrentRankData();
  }, [accessToken, editRankId]);
  ...
  return (
    <>
      ...
      {/* <div className="flex flex-col gap-2.5 2xl:gap-4 fixed z-20 w-full padding-responsive-header-container-type-1"> */}
      <div
        ...
        >
          ...
          <div className="flex items-center">
            ...
            >
              ...
              />
              <button className="text-bgPrimaryButton hover:text-bgPrimaryButtonHover font-Mona_Sans text-responsive-button font-semibold h-8 2xl:h-10">
                Back
              </button>
            </button>
          </div>
