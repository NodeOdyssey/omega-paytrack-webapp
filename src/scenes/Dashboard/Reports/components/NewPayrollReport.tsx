...
import { NewPayrollRow } from '../../../../types/newPayRollTypes';
...
const NewPayrollReport: React.FC<NewPayrollReportProps> = ({
  ...
  const [newPayrollReportData, setNewPayrollReportData] = useState<
    NewPayrollRow[]
  >([]);
  ...
  // console.log('newPayrollReportData', newPayrollReportData);
  // console.log('Access Token:', accessToken);
  // console.log('Selected Post Name:', selectedPostName);
  // console.log('Selected Date:', selectedDate);
  const fetchNewyPayrollReportData = async () => {
    if (!accessToken || !selectedPostName || !selectedDate) return;
    setIsLoading(true);
    // console.log(
    //   `${api.baseUrl}${api.reports}/new-payroll/${currentSelectedPostId}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`
    // );
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        setNewPayrollReportData(response.data.newPayrollReports);
  ...
  // const totalBasicSalary = newPayrollReportData.reduce(
  //   (sum, row) => sum + parseFloat(row.basicSalary),
  //   0
  // );
  // const totalNetPay = newPayrollReportData.reduce(
  ...
  //     ],
  //     body: newPayrollReportData.map((row, index) => [
  ...
  //       row.uniform,
  //       row.bonus,
  //       row.basicSalary + row.uniform + row.bonus,
  //       row.extraDuty,
  ...
  //   doc.setFont('Mona_Sans', 'normal');
  //   doc.setFontSize(10);
  //   doc.text(totalText, 110, doc.lastAutoTable.finalY + 10);
  //   doc.text(totalBasicSalaryText, 230, doc.lastAutoTable.finalY + 10);
  //   doc.text(totalNetPayText, 970, doc.lastAutoTable.finalY + 10);
  ...
  //   doc.save(`NewPayroll_Report_${formattedDate}_${formattedTime}.pdf`);
  // };
  ...
  //     doc.text(`For the period from: ${firstDay}`, 850, 80, { align: 'right' });
  //     doc.text(`To: ${lastDay}`, 950, 80, { align: 'right' });
  //   }
  ...
  //   // Footer Section
  //   const footerY = doc.lastAutoTable.finalY + 70;
  //   doc.setFontSize(12);
  //   doc.text('Rupees:', 350, footerY);
  //   doc.text(
  //     `${numberToWords(parseFloat(totalNetPay.toFixed(2)))} only`,
  ...
  const handleExportPDF = async () => {
    ...
    if (selectedDate) {
      const { firstDay, lastDay } = getFirstAndLastDay(selectedDate);
      doc.text(`For the period from: ${firstDay} To: ${lastDay}`, 830, 75, {
        align: 'right',
      });
    }
    ...
    autoTable(doc, {
      ...
      styles: {
        cellPadding: { top: 3, bottom: 3, left: 0.5, right: 0.5 },
        overflow: 'linebreak',
        // tableWidth: 'wrap',
      },
    ...
    doc.text(totalBasicSalaryText, 165, doc.lastAutoTable.finalY + 15);
    doc.text(totalNetPayText, 735, doc.lastAutoTable.finalY + 15);
  ...
  const csvHeaders = [
    ...
    { label: 'Employer EPF', key: 'empElrPF' },
    // { label: 'Advance Deduction', key: 'deduction.adv' },
    { label: 'P.Tax Deduction', key: 'pTax' },
  ...
  const csvData = newPayrollReportData.map((row, index) => ({
    ...
    deduction: {
      ...
      emplrESI: row.extraDuty,
      empElrPF: row.extraDuty,
    },
    // other: {
    //   belt: row.other.belt,
    //   boot: row.other.boot,
  ...
  //           <style>
  //             /* Add any styles you want to apply during print */
  ...
  const totalPages = Math.ceil(newPayrollReportData.length / rowsPerPage);

  // Get the data to display for the current page
  const currentPageData = newPayrollReportData.slice(
  ...
  const rowsToDisplay = isPrintMode ? newPayrollReportData : currentPageData;
  ...
  return (
    <>
      ...
      <div className="flex flex-col print:hidden">
                    ...
                    <ul className="p-2">
                      ...
                      >
                        <CSVLink
                          headers={csvHeaders}
                          data={csvData}
                          filename={`NewPayroll_Report_${formatCSVDate()}.csv`}
                        >
                          Export CSV
                        </CSVLink>
                      </li>
        ...
        {/* new payroll report table */}
        <div
          className="overflow-y-auto h-[calc(100vh-160px)] 2xl:h-[calc(100vh-220px)] overflow-x-auto mt-2 px-2 scrollbar scrollbar-thumb-scrollBarColor scrollbar-track-scrollBarBg"
          ref={tableRef}
        >
          {doesReportExist ? (
            <div>
              ...
              >
                ...
                <thead>
                  <tr>
                    <th
                      ...
                      colSpan={
                        ...
                        (visibleColumns.includes('emplrESI') ? 1 : 0) +
                        (visibleColumns.includes('pTax') ? 1 : 0) +
                  ...
                  <tr>
                    {visibleColumns.includes('empEPF') && (
                      <th className="border-2 border-primaryText text-xs  px-2 py-0.5 text-center  w-[10%]">
                        Employee EPF
                      </th>
                    )}
                ...
                <tbody className="text-center">
                  {/* {newPayrollReportData.map((row, rowIndex) => ( */}
                  {rowsToDisplay.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                      <td
                        className={`border-b border-dotted border-primaryText px-2 ${newPayrollReportData.length % 12 <= 2 ? 'py-6' : 'py-2'} text-xs `}
                      >
                        {isPrintMode
                      ...
                      <td className="border-b border-dotted border-primaryText px-2 py-2 text-xs text-right">
                      ...
                      <td className="border-b border-dotted border-primaryText px-2 py-2 text-xs text-right">
                        {/* {row.basicSalary} +{row.other.uniform} +{' '}
                      {row.allowances.kitAllowances} */}
                        {/* {row.basicSalary + row.uniform + row.bonus}
                         */}
                        {row.total.toFixed(2)}
                      </td>
                      ...
                      <td className="border-b border-dotted border-primaryText px-2 py-2 text-xs text-right">
                        {row.deduction.emplrEPF.toFixed(2)}
                      </td>
                      <td className="border-b border-dotted border-primaryText px-2 py-2 text-xs text-right">
                        {row.deduction.emplrESI.toFixed(2)}
                      </td>
                      <td className="border-b border-dotted border-primaryText px-2 py-2 text-xs text-right">
                      ...
                      <td colSpan={columns.length} className="p-4">
                        <div className="flex flex-col gap-4 items-center justify-center py-4">
                          <div className="flex items-center gap-4">
                            ...
                            <h3 className="text-primaryText reportPrimaryLabels2 uppercase">
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <h3 className="text-primaryText reportPrimaryLabels2">
                              Date:..........................
                            </h3>
                          </div>
                        </div>
              ...
              <div
                className={`flex flex-col gap-2 items-start font-bold w-[50%] mx-auto justify-center footer-to-print ${newPayrollReportData.length % 8 <= 2 ? 'py-4' : 'py-2'} `}
              >
                <div className="flex items-center gap-4">
