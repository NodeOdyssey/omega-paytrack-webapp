...
import { ESIRow } from '../../../../types/esiReportType';
...
const ESIReport: React.FC<ESIReportProps> = ({
  ...
  const [esiReportData, setEsiReportData] = useState<ESIRow[]>([]);
  const [doesReportExist, setDoesReportExist] = useState(false);

  const [totalGrossPay, setTotalGrossPay] = useState(0);
  const [grandTotalEsi, setGrandTotalEsi] = useState(0);

  const fetchEsiReportData = async () => {
    console.log('fetchEsiReportData');
    if (!accessToken || !selectedDate) return;
    setIsLoading(true);
    try {
      // const response = await axios.get(
      //   `${api.baseUrl}/reports/esi/${currentSelectedPostId}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`,
      //   {
      ...
      if (response.data && response.data.success) {
        ...
        console.log('ESI Reports:', response.data.esiReports);
        setEsiReportData(response.data.esiReports);
        setTotalGrossPay(Number(response.data.totalGrossPay));
        setGrandTotalEsi(Number(response.data.grandTotalEsi));
  ...
  // const totalGrossPay = esiReportData.reduce(
  //   (sum, row) => sum + parseFloat(row.grossPay),
  //   0
  // );
  // const totalNetPay = esiReportData.reduce(
  //   (sum, row) => sum + parseFloat(row.grossPay),
  //   0
  // );
  ...
  const handleExportPDF = async () => {
    ...
    autoTable(doc, {
      ...
      // body: esiReportData.map((row, index) => [
  ...
  const csvHeaders = [
    ...
    { label: 'Employees Contribution', key: 'empESI' },
    { label: 'Employers Contribution', key: 'emplrESI' },
    { label: 'Total', key: 'total' },
    { label: 'Sign', key: '' },
  ];

  // CSV Data
  const csvData = esiReportData.map((row, index) => ({
  ...
  const handlePrint = async () => {
    ...
    doc.setFontSize(10);
    doc.text(totalText, 30, doc.lastAutoTable.finalY + 10);
    doc.text(totalGrossPayText, 130, doc.lastAutoTable.finalY + 10);
    doc.text(grandTotalEsitText, 255, doc.lastAutoTable.finalY + 10);
    ...
    doc.text('Return after disbursement', 97, footerY + 10, {
      align: 'center',
    });
    doc.text(
      'Incharge: ..............................................................',
    ...
    // 👉 Instead of saving: directly print using iframe
  ...
  const totalPages = Math.ceil(esiReportData.length / rowsPerPage);

  // Get the data to display for the current page
  const currentPageData = esiReportData.slice(
  ...
  const rowsToDisplay = isPrintMode ? esiReportData : currentPageData;
    ...
    <>
      ...
      <div className="flex flex-col print:hidden">
        <div className="flex-none bg-tableHeadingColour">
          <div className="flex items-center justify-between px-2 2xl:px-4 border-t py-2 2xl:py-2">
            <div className="">
                    ...
                    <ul className="p-2">
                      ...
                      >
                        <CSVLink
                          headers={csvHeaders}
                          data={csvData}
                          filename={`ESI_Report_${formatCSVDate()}.csv`}
                        >
                          Export CSV
                        </CSVLink>
                      </li>
            ...
            <div>
              ...
              >
                ...
                <tbody className="text-center">
                  {/* {esiReportData.map((row, rowIndex) => ( */}
                  {rowsToDisplay.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                      <td
                        className={`border-b border-dotted border-primaryText px-4 ${esiReportData.length % 14 <= 2 ? 'py-3' : 'py-2'} text-xs `}
                      >
                        {isPrintMode
                      ...
                      <td
                        className={`border-b border-dotted border-primaryText text-right px-4 py-2 text-xs `}
                      >
                        {/* {row.accNo} */}
                      </td>
                      <td
                      ...
                      <td
                        className={`border-b border-dotted border-primaryText pr-6 py-2 text-right text-xs `}
                      >
                        {row.days}
                      </td>
                      <td
              ...
              <div
                className={`flex flex-col gap-2 items-start font-bold w-[50%] mx-auto justify-center footer-to-print ${esiReportData.length % 8 <= 2 ? 'py-4' : 'py-2'} `}
              >
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-primaryText reportPrimaryLabels2 uppercase font-bold">
                    {numberToWords(Number(grandTotalEsi.toFixed(2)))}
                  </h3>
                </div>
