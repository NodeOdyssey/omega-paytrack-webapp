...
import { DSLReportRow } from '../../../../types/DSLReportType';
...
const DSLReport: React.FC<DSLReportProps> = ({
  ...
  const [dslReportData, setDslReportData] = useState<DSLReportRow[]>([]);
  ...
  const fetchDsReportData = async () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        console.log('DSL Report:', response.data.dslReports);
        setDslReportData(response.data.dslReports);
  ...
  // const totalGrossPay = dslReportData.reduce(
  //   (sum, row) =>
  //     sum + parseFloat(row.eightHourPay + row.vda + row.hra + row.hra),
  //   0
  // );
  // const totalNetPay = dslReportData.reduce(
  ...
  const handleExportPDF = async () => {
    ...
    autoTable(doc, {
      ...
      body: dslReportData.map((row, index) => [
        index + 1,
        `${row.empName}\n${row.rank}`,
        row.days,
        row.eightHourPay,
        ...
        row.uniform,
        row.hra,
        row.eightHourPay + row.uniform + row.hra + row.vda,
        row.extraDuty,
        row.adv,
    ...
    doc.text(totalGrossPayText, 470, doc.lastAutoTable.finalY + 20);
    doc.text(totalNetPayText, 920, doc.lastAutoTable.finalY + 20);
    ...
    doc.save(`DSL_Report_${formattedDate}_${formattedTime}.pdf`);
  };
  ...
  const csvHeaders = [
    ...
    { label: 'Employer ESI', key: 'deduction.empESI' },
    { label: 'Employer EPF', key: 'deduction.empEPF' },
  ...
  const csvData = dslReportData.map((row, index) => ({
    ...
    uniform: row.uniform,
    hra: row.hra,
    total: row.eightHourPay + row.vda + row.uniform + row.hra,
  ...
  const totalPages = Math.ceil(dslReportData.length / rowsPerPage);

  // Get the data to display for the current page
  const currentPageData = dslReportData.slice(
  ...
  const rowsToDisplay = isPrintMode ? dslReportData : currentPageData;
                    ...
                    <ul className="p-2">
                      ...
                      >
                        <CSVLink
                          headers={csvHeaders}
                          data={csvData}
                          filename={`DSL_Report_${formatCSVDate()}.csv`}
                        >
                          Export CSV
                        </CSVLink>
                      </li>
        ...
        >
          {doesReportExist ? (
            <div>
              ...
              >
                ...
                <thead>
                  ...
                  <tr>
                    <th
                      ...
                      colSpan={
                        ...
                        (visibleColumns.includes('hra') ? 1 : 0) +
                  ...
                  <tr>
                    ...
                    {visibleColumns.includes('empName') && (
                      <th
                        className="border-2 border-primaryText px-2 py-0.5 text-left text-xs  w-[20%]"
                        rowSpan={4}
                      >
                        Name & Rank
                      </th>
                ...
                <tbody className="text-center">
                  {/* {dslReportData.map((row, rowIndex) => ( */}
                  {rowsToDisplay.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                      <td
                        className={`border-b border-dotted border-primaryText px-2 ${dslReportData.length % 12 <= 2 ? 'py-6' : 'py-2'}  text-xs `}
                      >
                        {isPrintMode
                      ...
                      {/* total */}
                      <td className="border-b border-dotted border-primaryText px-2 py-2 text-xs text-right">
                        {/* {row.eightHourPay + row.uniform + row.vda + row.hra} */}
                        {row.total}
                      </td>
                      {/* extra duty */}
                      <td className="border-b border-dotted border-primaryText px-2 py-2 text-xs text-right">
                      ...
                      <td className="border-b border-dotted border-primaryText pr-1 py-2 text-xs ">
              ...
              <div
                className={`flex flex-col gap-2 items-start font-bold w-[50%] mx-auto justify-center footer-to-print ${dslReportData.length % 8 <= 2 ? 'py-4' : 'py-2'} `}
              >
