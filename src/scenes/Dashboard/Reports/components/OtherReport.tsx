...
import { OtherReportRow } from '../../../../types/otherReportType';
import Loader from '../../../../common/Loader/Loader';

interface OtherReportProps {
  currentSelectedPostId: number;
  selectedPostName: string | null;
  selectedDate: Date | null;
}
const OtherReport: React.FC<OtherReportProps> = ({
  ...
  const [otherReportData, setOtherReportData] = useState<OtherReportRow[]>([]);
  ...
  const fetchDsReportData = async () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        // console.log('Reports:', response.data.otherReports);
        setOtherReportData(response.data.otherReports);
        setTotalGrossPay(Number(response.data.totalGrossPay));
  ...
  // const totalGrossPay = otherReportData.reduce(
  //   (sum, row) => sum + parseFloat(row.grossPay),
  //   0
  // );
  // const totalNetPay = otherReportData.reduce(
  ...
  const handleExportPDF = async () => {
    ...
    autoTable(doc, {
      ...
      head: [
        [
          { content: 'Sl No.', rowSpan: 1 },
          { content: 'Name', rowSpan: 1 },
          { content: 'Days', rowSpan: 1 },
          { content: 'Basic Pay', rowSpan: 1 },
          { content: 'Allowances', rowSpan: 1 },
          { content: 'Gross Pay', rowSpan: 1 },
          { content: 'Extra Duty', rowSpan: 1 },
          { content: 'Uniform', rowSpan: 1 },
          { content: '4 Hour Pay', rowSpan: 1 },
          { content: 'Special Allowance', rowSpan: 1 },
          { content: 'Bonus', rowSpan: 1 },
          { content: 'Deduction', rowSpan: 1 },
          { content: 'Net Pay', rowSpan: 1 },
          { content: 'Sign', rowSpan: 1 },
        ],
      ],
      body: otherReportData.map((row, index) => [
        index + 1, // Serial Number (Sl. No.)
        row.empName,
        row.days,
        row.basicSalary,
        `Kits/Washing: ${row.allowance.kitAllowances}, \n City Allow: ${row.allowance.cityAllowances}, \n Conv/H.Rent: ${row.allowance.convHra}`,
        row.grossPay,
        row.extraDuty,
        row.uniform,
        // four hour pay
        ...
        // bonus
        row.bonus,
        // Deduction (Formatted to show left and right side content)
        `ESI: ${row.deduction.empESI}, Adv: ${row.deduction.adv}\n` +
    ...
    doc.text(totalGrossPayText, 440, doc.lastAutoTable.finalY + 20);
    doc.text(totalNetPayText, 1010, doc.lastAutoTable.finalY + 20);
    ...
    doc.save(`Others_Report_${formattedDate}_${formattedTime}.pdf`);
  };
  ...
  const csvHeaders = [
    ...
    { label: '8 Hours Pay', key: 'basicSalary' },
    { label: 'Kit Washings Allowance', key: 'kitWashing' },
    { label: 'City Allowance', key: 'cityAllowances' },
    { label: 'Conv/H.Rent', key: 'convHra' },
    { label: 'Gross Pay', key: 'grossPay' },
    { label: 'Extra Duty', key: 'extraDuty' },
    { label: 'Uniform', key: 'uniform' },
    { label: '4 Hours Pay', key: 'fourHourPay' },
    { label: 'Special Allowances', key: 'specialAllowance' },
    { label: 'Bonus', key: 'bonus' },
    { label: 'Employee ESI', key: 'empESI' },
    { label: 'Employee EPF', key: 'empEPF' },
    { label: 'Belt', key: 'belt' },
    { label: 'Uniform', key: 'uniform1' },
    // { label: 'Adv', key: 'deduction.adv' },
    // { label: 'P.Tax', key: 'deduction.pTax' },
  ...
  const csvData = otherReportData.map((row, index) => ({
    slNo: index + 1, // Serial number
    empName: row.empName,
    days: row.days,
    basicSalary: row.basicSalary,
    kitWashing: row.allowance.kitAllowances,
    cityAllowances: row.allowance.cityAllowances,
    convHra: row.allowance.convHra,
    grossPay: row.grossPay,
    extraDuty: row.extraDuty,
    uniform: row.uniform,
    fourHourPay: row.fourHourPay,
    specialAllowance: row.specialAllowance,
    bonus: row.bonus,
    empESI: row.deduction.empESI,
    empEPF: row.deduction.empEPF,
    belt: row.deduction.belt,
    uniform1: row.deduction.Uniform,
    // total:
    //   row.basicSalary +
    //   row.deduction.pTax +
    //   row.other.uniform +
    //   row.allowances.convHra,
  ...
  const totalPages = Math.ceil(otherReportData.length / rowsPerPage);

  // Get the data to display for the current page
  const currentPageData = otherReportData.slice(
  ...
  const rowsToDisplay = isPrintMode ? otherReportData : currentPageData;
                    ...
                    <ul className="p-2">
                      ...
                      >
                        <CSVLink
                          headers={csvHeaders}
                          data={csvData}
                          filename={`Others_Report_${formatCSVDate()}.csv`}
                        >
                          Export CSV
                        </CSVLink>
                      </li>
            ...
            <div>
              ...
              >
                ...
                <colgroup>
                  ...
                  {visibleColumns.includes('fourHourPay') && (
                    <col style={{ width: '10%' }} />
                  )}
                ...
                <thead>
                  ...
                  <tr>
                    <th
                      className="border-2 border-primaryText px-2 py-0.5 text-center text-xs  w-[5%]"
                      rowSpan={2}
                    >
                      Sl. No.
                    </th>
                    {visibleColumns.includes('empName') && (
                      <th
                        className="border-2 border-primaryText px-2 py-0.5 text-left text-xs   w-[10%]"
                        rowSpan={2}
                      >
                        Name
                      </th>
                    ...
                    {visibleColumns.includes('allowance') && (
                      <th
                        className="border-2 border-primaryText px-2 py-0.5 text-center text-xs   w-[20%]"
                        rowSpan={2}
                      >
                        Allowances
                      </th>
                ...
                <tbody className="text-center">
                  {/* {otherReportData.map((row, rowIndex) => ( */}
                  {rowsToDisplay.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                      <td
                        className={`border-b border-dotted border-primaryText px-2 ${otherReportData.length % 8 <= 2 ? 'py-10' : 'py-1'} text-xs `}
                      >
                        {isPrintMode
                          ? rowIndex + 1 // Show correct serial number for all rows in print mode
                          : (currentPage - 1) * rowsPerPage + rowIndex + 1}
                      </td>
                      <td className="border-b border-dotted border-primaryText text-left px-2 py-1 text-xs  whitespace-nowrap">
                        {row.empName}
                      </td>
                      <td className="border-b border-dotted border-primaryText pr-4 py-1 text-xs text-right">
                        {row.days}
                      </td>
                      {/* Basic Pay */}
                      ...
                      <td className="border-b border-dotted border-primaryText pr-2 py-1 text-xs text-right ">
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex justify-between space-x-2">
                            ...
                            <h3 className="text-xs font-bold">
                              {row.allowance.cityAllowances}
                            </h3>
                          </div>
                          <div className="flex justify-between space-x-2">
                            <h3 className="text-xs  font-bold">Conv/H.Rent:</h3>
                            <h3 className="text-xs font-bold">
                              {row.allowance.convHra}
                            </h3>
                          </div>
                        </div>
                      </td>
                  ...
                  <tr className="mt-2 mx-auto">
                    ...
                    <td className="border-t-2 border-primaryText px-2 py-2 font-bold"></td>
                    <td className="border-t-2 border-primaryText px-2 py-2 font-bold"></td>
                    <td className="border-t-2 border-primaryText  py-2 font-bold text-xs  text-right">
              ...
              {/* disburstment */}
              <div
                className={`flex flex-col gap-2 items-start font-bold w-[50%] mx-auto justify-center footer-to-print ${otherReportData.length % 8 <= 2 ? 'py-4' : 'py-2'} `}
              >
