...
import './reports.css';
// import { SalaryRow } from '../../../../types/salaryReportType';
...
const SalaryReport: React.FC<SalaryReportProps> = ({
  ...
  const [salaryReportData, setSalaryReportData] = useState<SalaryRow[]>([]);
  ...
  const fetchEpfReportData = async () => {
    ...
    try {
      ...
      //   `${api.baseUrl}/reports/salary/${currentSelectedPostId}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`,
      //   {
      ...
      if (response.data && response.data.success) {
        ...
        console.log('Salary Reports:', response.data);
        setSalaryReportData(response.data.salaryReport);
  ...
  // const totalNetPay = salaryReportData.reduce(
  ...
  const handleExportPDF = async () => {
    ...
    autoTable(doc, {
      ...
      // body: salaryReportData.map((row, index) => [
      ...
      //       return `ESI: ${row.deduction.empESI}, Salary: ${row.deduction.empSalary}, Adv: ${row.deduction.adv}, P.Tax: ${row.deduction.pTax}`;
    ...
    const totalGrossPayText = ` ${totalNetPay}`;
  ...
  // CSV Data
  const csvData = salaryReportData.map((row, index) => ({
    slNo: index + 1, // Serial number
    empName: row.empName,
    accNo: row.accountNum,
    ifsc: row.ifsc,
  ...
  const totalPages = Math.ceil(salaryReportData.length / rowsPerPage);

  // Get the data to display for the current page
  const currentPageData = salaryReportData.slice(
  ...
  const rowsToDisplay = isPrintMode ? salaryReportData : currentPageData;
                    ...
                    <ul className="p-2">
                      ...
                      >
                        <CSVLink
                          headers={csvHeaders}
                          data={csvData}
                          filename={`Salary_Report_${formatCSVDate()}.csv`}
                        >
                          Export CSV
                        </CSVLink>
                      </li>
            ...
            <div>
              ...
              >
                ...
                <thead>
                  ...
                  <tr>
                    <th
                      ...
                      colSpan={
                        ...
                        (visibleColumns.includes('ifsc') ? 1 : 0) +
                        (visibleColumns.includes('bankName') ? 1 : 0) +
                        (visibleColumns.includes('amount') ? 1 : 0) +
                  ...
                  {/* second tr */}
                  <tr>
                    <th className="border-2 border-primaryText px-4 text-center text-xs  w-[10%]">
                      Sl. No.
                    </th>
                    {/* <th
                        className="border-2 border-primaryText px-4 text-center text-xs  w-[10%]"
                        rowSpan={2}
                      >
                        ESI No.
                      </th> */}
                    {visibleColumns.includes('empName') && (
                      <th
                        className="border-2 border-primaryText px-4 text-left text-xs  w-[10%]"
                        rowSpan={2}
                      >
                        Name
                      </th>
                ...
                <tbody className="text-center">
                  {/* {salaryReportData.map((row, rowIndex) => ( */}
                  {rowsToDisplay.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                      <td
                        className={`border-b border-dotted border-primaryText px-4 ${salaryReportData.length % 14 <= 2 ? 'py-3' : 'py-2'} text-xs `}
                      >
                        {isPrintMode
                      ...
                      <td
                        className={`border-b border-dotted border-primaryText py-2 text-xs`}
                      >
                        {row.bankName}
                      </td>
                      <td
                        className={`border-b border-dotted border-primaryText pr-4 py-2 text-xs text-right`}
                      >
                        {row.netPay}
                      </td>
              ...
              {/* disbursment */}
              <div
                className={`flex flex-col gap-2 items-start font-bold w-[50%] mx-auto justify-center footer-to-print ${salaryReportData.length % 8 <= 2 ? 'py-4' : 'py-2'} `}
              >
                <div className="flex items-center gap-4">
