...
const PTaxReport: React.FC<PTaxReportProps> = ({
  ...
  const alwaysVisibleColumns = ['empName', 'grossPay', 'pTax', 'postName'];
  ...
  const columns = [
    ...
    { name: 'grossPay', label: 'Gross Salary' },
    { name: 'pTax', label: 'Professional Tax' },
    { name: 'postName', label: 'Post Name' },
    { name: 'sign', label: 'Sign' },
  ];

  const [pTaxReportData, setPtaxReportData] = useState<PTaxRow[]>([]);
  ...
  const [grandTotalPtax, setGrandTotalPtax] = useState(0);

  const fetchPtaxReportData = async () => {
    console.log('fetchPTaxReportData');
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        console.log('PTax Reports:', response.data);
        setPtaxReportData(response.data.pTaxReport);
        // setTotalGrossPay(Number(response.data.totalGrossPay));
        setTotalBasicSalary(Number(response.data.totalBasicSalary));
        setGrandTotalPtax(Number(response.data.totalPTax));
  ...
  function numberToWords(num: number) {
    ...
    console.log('Number to words result:', result); // TODO: Remove console.log
    return result;
  }
  ...
  const handleExportPDF = async () => {
    ...
    autoTable(doc, {
      ...
      body: pTaxReportData.map((row, index) => [
        index + 1,
        row.empName,
        // row.grossPay,
        row.basicSalary,
    ...
    // const grandTotalPtaxtText = ` ${grandTotalPtax}`;

    doc.setFont('Mona_Sans', 'normal');
    doc.setFontSize(10);
    doc.text(totalText, 30, doc.lastAutoTable.finalY + 10);
    // doc.text(totalGrossPayText, 120, doc.lastAutoTable.finalY + 10);
    doc.text(totalBasicSalaryText, 255, doc.lastAutoTable.finalY + 10);
    ...
    // doc.text(
    //   `${numberToWords(parseFloat(totalGrossPay.toFixed(2)))} only`,
    //   88,
    //   footerY
    // );
  ...
  const csvHeaders = [
    ...
    { label: 'Gross Salary', key: 'grossPay' },
    { label: 'Professional Tax', key: 'pTax' },
    { label: 'Post Name', key: 'postName' },
  ];
  // CSV Data
  const csvData = pTaxReportData.map((row, index) => ({
    slNo: index + 1, // Serial number
    empName: row.empName,
    // accNo: row.grossPay,
    accNo: row.basicSalary,
    ifsc: row.pTax,
  ...
  const totalPages = Math.ceil(pTaxReportData.length / rowsPerPage);

  // Get the data to display for the current page
  const currentPageData = pTaxReportData.slice(
  ...
  const rowsToDisplay = isPrintMode ? pTaxReportData : currentPageData;
                    ...
                    <ul className="p-2">
                      ...
                      >
                        <CSVLink
                          headers={csvHeaders}
                          data={csvData}
                          filename={`PTax_Report_${formatCSVDate()}.csv`}
                        >
                          Export CSV
                        </CSVLink>
                      </li>
            ...
            <div>
              ...
              >
                ...
                <colgroup>
                  ...
                  {visibleColumns.includes('postName') && (
                    <col style={{ width: '30%' }} />
                  )}
                ...
                <thead>
                  ...
                  <tr>
                    <th
                      ...
                      colSpan={
                        ...
                        (visibleColumns.includes('postName') ? 1 : 0) +
                        1 // Sign
                      }
                    >
                  ...
                  {/* second tr */}
                  <tr>
                    <th className="border-2 border-primaryText px-4 py-0.5 text-center text-xs   w-[10%]">
                      Sl. No.
                    </th>
                    {visibleColumns.includes('empName') && (
                      <th
                        className="border-2 border-primaryText px-4 py-0.5 text-left text-xs   w-[10%]"
                        rowSpan={2}
                      >
                        Name
                      </th>
                    ...
                    {visibleColumns.includes('grossPay') && (
                      <th
                        className="border-2 border-primaryText px-4 py-0.5 text-center text-xs   w-[10%]"
                        rowSpan={2}
                      >
                        Gross Salary
                      </th>
                ...
                <tbody className="text-center">
                  {rowsToDisplay.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                      <td
                        className={`border-b border-dotted border-primaryText px-4 ${pTaxReportData.length % 14 <= 2 ? 'py-3' : 'py-2'} text-xs `}
                      >
                        {isPrintMode
                          ? rowIndex + 1
                      ...
                      <td
                        className={`border-b border-dotted border-primaryText pr-2 py-2 text-xs text-right`}
                      >
                        {/* {row.grossPay} */}
                        {row.basicSalary}
                        {/* gross pay */}
                      ...
                      <td
                        className={`border-b border-dotted border-primaryText pr-2 py-3 text-xs text-right`}
                      >
                        {row.pTax}
                        {/* professional tax */}
                      </td>
                      <td
                        className={`border-b border-dotted text-left border-primaryText pl-2 py-3 text-xs`}
                      >
                        {row.postName}
                      </td>
                      <td
                        className={`border-b border-dotted border-primaryText px-4 py-3 text-xs `}
                      ></td>
                    </tr>
                  ))}
                  {/* total */}
                  <tr className="mt-2 mx-auto">
                    <td className="border-t-2 border-primaryText px-4 py-3 font-bold"></td>
                    <td className="border-t-2 border-primaryText px-4 py-3 font-bold text-xs md:text-sm ">
                      Total
                    </td>
                    {/* gross Salary */}
                    {/* <td className="border-t-2 border-primaryText px-4 py-3 font-bold text-xs md:text-sm text-right">
              ...
              <div
                className={`flex flex-col gap-2 items-start font-bold w-[50%] mx-auto justify-center footer-to-print ${pTaxReportData.length % 8 <= 2 ? 'py-4' : 'py-2'} `}
              >
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-primaryText reportPrimaryLabels2 uppercase font-bold">
                    {numberToWords(Number(grandTotalPtax.toFixed(2)))}
                  </h3>
                </div>
