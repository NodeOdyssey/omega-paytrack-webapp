...
type EmployeePromoteModalProps = {
  message: string;
  onConfirm: (e: React.FormEvent) => void;
  onCancel: () => void;
  updateNewSelectedPostRankId: (postRankLinkId: number) => void;
  updatePromotePostingDate: (date: Date | null) => void;
  confirmButtonTitle: string;
  cancelButtonTitle?: string;
};

// Validation schema for schedule employee data row
const promoteEmployeeValidationSchema = Yup.object().shape({
...
const EmployeePromoteModal: React.FC<EmployeePromoteModalProps> = ({
  message,
  onConfirm,
  onCancel,
  updateNewSelectedPostRankId,
  updatePromotePostingDate,
  confirmButtonTitle,
  cancelButtonTitle,
}) => {
  /* Verify User Auth */
  ...
  const handleSaveClick = async (e: React.FormEvent) => {
    ...
    try {
      await promoteEmployeeValidationSchema.validate(
        {
          currentSelectedPostId,
  ...
  useEffect(() => {
    ...
    // console.log('promotePostingDate', promotePostingDate);
  });
                  ...
                  <tr className="border border-tableBorder h-16">
                    <td className="px-2 py-2 h-10 text-xs lg:text-sm xl:text-base border border-tableBorder">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          // setPromotePostingDate(new Date());
                          setAllPostRankLinksData([]);
                        }}
                    ...
                    <td className="px-2 py-2 h-10 text-xs lg:text-sm xl:text-base border border-tableBorder">
                      <input
                        ...
                        onChange={(e) => {
                          updatePromotePostingDate(new Date(e.target.value));
                          setPromotePostingDate(new Date(e.target.value));
                        }}
                        className="w-full h-full bg-white border rounded-lg py-2 px-2 max-h-60 cursor-pointer"
                      />
                    </td>
...
export default EmployeePromoteModal;
