...
type EmployeeAbscondModalProps = {
  message: string;
  onConfirm: (e: React.FormEvent) => void;
  onCancel: () => void;
  updateAbscondDate: (date: Date | null) => void;
  confirmButtonTitle: string;
  cancelButtonTitle?: string;
};

// Validation schema for schedule employee data row
const abscondEmployeeValidationSchema = Yup.object().shape({
  abscondDate: Yup.date()
    .required('Please enter the date of abscond')
    .test('not-2000', 'Please select a valid date of abscond', (value) => {
      return value && value.getFullYear() !== 2000;
    }),
});

const EmployeeAbscondModal: React.FC<EmployeeAbscondModalProps> = ({
  message,
  onConfirm,
  onCancel,
  updateAbscondDate,
  ...
  /* Dishcarge Controls */
  const [abscondDate, setAbscondDate] = useState<Date | null>(null);
  ...
  const handleSaveClick = async (e: React.FormEvent) => {
    ...
    try {
      await abscondEmployeeValidationSchema.validate(
        {
          abscondDate,
        },
        { abortEarly: false }
                  ...
                  <tr className="border border-tableBorder h-16">
                    <td className="px-2 py-2 h-10 text-xs lg:text-sm xl:text-base border border-tableBorder">
                      <input
                        ...
                        id="date"
                        onChange={(e) => {
                          updateAbscondDate(new Date(e.target.value));
                          setAbscondDate(new Date(e.target.value));
                        }}
...
export default EmployeeAbscondModal;
