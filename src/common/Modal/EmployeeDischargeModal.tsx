...
type EmployeeDischargeModalProps = {
  message: string;
  onConfirm: (e: React.FormEvent) => void;
  onCancel: () => void;
  updateDischargePostingDate: (date: Date | null) => void;
  confirmButtonTitle: string;
  cancelButtonTitle?: string;
};

// Validation schema for schedule employee data row
const dischargeEmployeeValidationSchema = Yup.object().shape({
  dischargePostingDate: Yup.date()
    .required('Please enter the date of discharge')
    .test('not-2000', 'Please select a valid date of discharge', (value) => {
      return value && value.getFullYear() !== 2000;
    }),
});

const EmployeeDischargeModal: React.FC<EmployeeDischargeModalProps> = ({
  message,
  onConfirm,
  onCancel,
  updateDischargePostingDate,
  confirmButtonTitle,
  cancelButtonTitle,
}) => {
  /* Verify User Auth */
  ...
  const handleSaveClick = async (e: React.FormEvent) => {
    ...
    try {
      await dischargeEmployeeValidationSchema.validate(
        {
          dischargePostingDate,
        },
        { abortEarly: false }
      );
                  ...
                  <tr className="border border-tableBorder h-16">
                    <td className="px-2 py-2 h-10 text-xs lg:text-sm xl:text-base border border-tableBorder">
                      <input
                        ...
                        id="date"
                        onChange={(e) => {
                          updateDischargePostingDate(new Date(e.target.value));
                          setDischargePostingDate(new Date(e.target.value));
                        }}
...
export default EmployeeDischargeModal;
